...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const cat = await categories.get(params.slug)
    return {
      title: cat.name,
...
export default async function CategoryPage({ params }: Props) {
  const cat = await categories.get(params.slug).catch(() => null)
  if (!cat) notFound()

  const data = await listings.list({ category: params.slug }).catch(() => ({ results: [], count: 0 }))

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      {/* Header */}
      <div className="bg-gradient-to-br from-primary-950 to-primary-800 text-white rounded-3xl px-8 py-12 mb-10">
